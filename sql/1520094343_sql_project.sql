...
SELECT COUNT(name) AS free_facility_count FROM Facilities WHERE membercost = 0
...
SELECT name, monthlymaintenance, CASE WHEN monthlymaintenance > 100 THEN 'expensive' ELSE 'cheap' END AS maint_cost_description FROM Facilities
...
SELECT name, full_name, booking_cost 
FROM 
    (SELECT B.starttime, F.name, CONCAT(M.surname, ', ', M.firstname) AS full_name, B.bookid, CASE WHEN M.memid =0 THEN (F.guestcost * B.slots) ELSE (F.membercost * B.slots) END AS booking_cost
     FROM Bookings B
     INNER JOIN Facilities F ON F.facid = B.facid
     INNER JOIN Members M ON M.memid = B.memid
    ) sub
...
SELECT total_slots_table.facid, total_guest_slots, total_memb_slots, ((F.membercost * total_memb_slots) + (F.guestcost * total_guest_slots)) AS rev_per_fac
FROM 
    (SELECT Bookings.facid, SUM(slots) AS total_guest_slots, total_memb_slots
     FROM Bookings
     INNER JOIN 
         (SELECT facid, SUM(slots) AS total_memb_slots
         FROM Bookings
         WHERE memid != 0
         GROUP BY facid
        ) memb_slot_table
